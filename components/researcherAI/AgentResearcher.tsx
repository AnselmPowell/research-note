...
export const AgentResearcher: React.FC = () => {
    ...
    // NEW: Append research results to "My Results" tab when research completes
    // Uses ref to trigger only once per research cycle (avoid duplicates)
    useEffect(() => {
        console.log('\n╔════════════════════════════════════════════════════════════════╗');
        console.log('║ [AGENT-APPEND] Append Trigger useEffect - Checking conditions  ║');
        console.log('╚════════════════════════════════════════════════════════════════╝');
        
        console.log('[AGENT-APPEND] 📋 Current State:', {
            researchPhase,
            hasAppendedBefore: hasAppendedResultsRef.current,
            deepResearchResultsCount: deepResearchResults.length,
            deepResearchResultsPdfUris: deepResearchResults.map(n => n.pdfUri),
            deepResearchResultsPages: deepResearchResults.map(n => n.pageNumber),
            processedPdfsCount: processedPdfs.length,
            processedPdfsUris: processedPdfs.map(p => p.uri),
            selectedArxivPapersCount: selectedArxivPapers.length,
            selectedArxivPaperIds: selectedArxivPapers.map(p => p.id).slice(0, 3)
        });

        if (researchPhase === 'completed' && !hasAppendedResultsRef.current) {
            hasAppendedResultsRef.current = true;
            
            console.log('\n[AGENT-APPEND] ✅ Research COMPLETED! Phase is "completed" and haven\'t appended yet');
            console.log('[AGENT-APPEND] 🔍 Starting append logic...\n');
            
            // Get ArXiv papers - ALL of them, not just those with notes
            // Notes will appear if they exist, otherwise paper shows with empty notes section
            console.log('[AGENT-APPEND] 📄 ArXiv Papers Analysis:', {
                total: selectedArxivPapers.length,
                withNotes: selectedArxivPapers.filter(p => p.notes?.length > 0).length,
                papers: selectedArxivPapers.map(p => ({ id: p.id, title: p.title, notesCount: p.notes?.length || 0 })).slice(0, 3)
            });
            
            // CHANGED: Include ALL ArXiv papers, not just those with notes
            const arxivPapersWithNotes = selectedArxivPapers;
            
            // NEW: Convert selected web search results to paper objects for "My Results"
            // Only include web results that were explicitly selected (same pattern as ArXiv)
            const webSearchPapers = (searchState.data?.sources || [])
                .filter(source => selectedWebSourceUris.has(source.uri))
                .map((source: any) => ({
                    id: source.uri,
                    title: source.title,
                    authors: [],
                    ...
                    notes: deepResearchResults.filter(n => n.pdfUri === source.uri),
                    analysisStatus: 'completed' as const,
                    harvardReference: '',
                    keywords: [],
                    sourceApi: 'web_search' as const,
                    relevanceScore: 0.5,
                }));
            
            console.log('[AGENT-APPEND] 🌐 Web Search Results Analysis:', {
                total: searchState.data?.sources?.length || 0,
                selected: webSearchPapers.length,
                papers: webSearchPapers.map(p => ({ id: p.id, title: p.title, notesCount: p.notes?.length || 0 })).slice(0, 3)
            });
            
            // CRITICAL FIX: Use processedPdfs (papers that were actually analyzed)
            // NOT contextPdfs (which is recalculated and may be different)
            // CRITICAL FIX: If processedPdfs is empty but we have deepResearchResults,
            // it means Source Panel PDFs were analyzed but processedPdfs state is stale
            // Extract PDF info from the notes themselves
            const pdfUrisFromNotes = new Set(deepResearchResults.map(n => n.pdfUri));
            const reconstructedPdfs = Array.from(pdfUrisFromNotes).map(uri => ({
                uri,
                // We don't have full PDF object, but we can create minimal object for matching
                // The important part is the URI for matching notes
            }));

            console.log('\n[AGENT-APPEND] 📎 PDF Papers Analysis:');
            console.log('[AGENT-APPEND] processedPdfs from state:', processedPdfs.length);
            console.log('[AGENT-APPEND] Unique PDF URIs from notes:', pdfUrisFromNotes.size);
            
            // Use reconstructed PDFs if processedPdfs is empty but notes exist
            const pdfsToMap = processedPdfs.length > 0 ? processedPdfs : (reconstructedPdfs as any);
            console.log('[AGENT-APPEND] Processing', pdfsToMap.length, 'PDFs...');
            
            const pdfPapersWithNotes = pdfsToMap.map((pdf: any) => {
                console.log(`[AGENT-APPEND] 🔍 Mapping PDF: ${pdf.uri}`);
                const notesForThisPdf = deepResearchResults.filter(n => n.pdfUri === pdf.uri);
                console.log(`[AGENT-APPEND]    ↳ Found ${notesForThisPdf.length} notes for this PDF`);
                
                // CRITICAL FIX: Get title from loadedPdfs first, then metadata, then fallback
                // This ensures we use the display title from Source Panel
                const loadedPdfEntry = loadedPdfs.find(lp => lp.uri === pdf.uri);
                const hasMetadata = pdf.metadata && pdf.file;
                
                let title = 'Untitled Paper';
                
                // Priority 1: Use title from loaded PDFs (this is what was shown in Source Panel)
                if (loadedPdfEntry?.metadata?.title) {
                    title = loadedPdfEntry.metadata.title;
                    console.log(`[AGENT-APPEND]    ↳ Using title from LoadedPdf metadata: "${title}"`);
                } 
                // Priority 2: Use file name from LoadedPdf
                else if (loadedPdfEntry?.file?.name) {
                    title = loadedPdfEntry.file.name.replace('.pdf', '');
                    console.log(`[AGENT-APPEND]    ↳ Using title from LoadedPdf file name: "${title}"`);
                }
                // Priority 3: Use metadata title from processedPdfs
                else if (hasMetadata && pdf.metadata.title) {
                    title = pdf.metadata.title;
                    console.log(`[AGENT-APPEND]    ↳ Using title from processedPdf metadata: "${title}"`);
                }
                // Priority 4: Use file name from processedPdfs
                else if (hasMetadata && pdf.file?.name) {
                    title = pdf.file.name.replace('.pdf', '');
                    console.log(`[AGENT-APPEND]    ↳ Using title from processedPdf file name: "${title}"`);
                }
                // Priority 5: Last resort - URI substring (should rarely happen now)
                else {
                    title = `Paper from ${pdf.uri.substring(0, 50)}...`;
                    console.log(`[AGENT-APPEND]    ↳ Using fallback title from URI: "${title}"`);
                }
                
                return {
                    id: pdf.uri,
                    title: title,
                    authors: (hasMetadata && pdf.metadata.author) ? [pdf.metadata.author] : [],
                    pdfUri: pdf.uri,
                    summary: (hasMetadata && pdf.metadata.subject) ? pdf.metadata.subject : '',
                    publishedDate: (hasMetadata && pdf.metadata.creationDate) ? pdf.metadata.creationDate : new Date().toISOString(),
                    notes: notesForThisPdf,
                    analysisStatus: (uploadedPaperStatuses[pdf.uri] || 'completed') as any,
                    harvardReference: '',
                    keywords: [],
                } as any;
            }); // CHANGED: Removed .filter(p => p.notes && p.notes.length > 0)
            // Now ALL papers are included, whether they have notes or not
            
            console.log('\n[AGENT-APPEND] 📋 PDF Papers with Notes:', {
                total: pdfsToMap.length,
                withNotes: pdfPapersWithNotes.length,
                papers: pdfPapersWithNotes.map(p => ({ 
                    uri: p.pdfUri, 
                    title: p.title, 
            ...
            const allPapersToAdd = [...arxivPapersWithNotes, ...pdfPapersWithNotes, ...webSearchPapers];
            
            console.log('\n[AGENT-APPEND] 🎯 Final Papers to Append to My Results:', {
                totalPapers: allPapersToAdd.length,
                arxivCount: arxivPapersWithNotes.length,
                pdfCount: pdfPapersWithNotes.length,
                webCount: webSearchPapers.length,
                totalNotes: deepResearchResults.length,
                papersDetails: allPapersToAdd.map(p => ({
                    id: p.id,
                    title: p.title,
                    source: p.sourceApi || 'unknown',
                    notesCount: p.notes?.length || 0
                }))
            });
            
            if (allPapersToAdd.length > 0) {
                console.log('\n[AGENT-APPEND] 🚀 Calling addToPaperResults with:', {
                    papersCount: allPapersToAdd.length,
                    notesCount: deepResearchResults.length,
                    papers: allPapersToAdd.map(p => ({ id: p.id, title: p.title, notesCount: p.notes?.length || 0 })),
                    notes: deepResearchResults.map(n => ({ pdfUri: n.pdfUri, pageNumber: n.pageNumber }))
                });
                addToPaperResults(allPapersToAdd, deepResearchResults);
                console.log('[AGENT-APPEND] ✅ addToPaperResults called successfully');
            } else {
                console.log('[AGENT-APPEND] ⚠️  NO PAPERS TO ADD!', {
                    arxivCount: arxivPapersWithNotes.length,
                    pdfCount: pdfPapersWithNotes.length,
                    totalCount: allPapersToAdd.length,
                    deepResearchResultsEmpty: deepResearchResults.length === 0,
                    processedPdfsEmpty: processedPdfs.length === 0
                });
            }
            
            console.log('╔════════════════════════════════════════════════════════════════╗');
            console.log('║ [AGENT-APPEND] Append Trigger useEffect - COMPLETE            ║');
            console.log('╚════════════════════════════════════════════════════════════════╝\n');
        }
    }, [researchPhase, deepResearchResults, processedPdfs, addToPaperResults, uploadedPaperStatuses, selectedArxivPapers]);

    // Reset append flag when research starts (CRITICAL: must be at START, not at END)
    useEffect(() => {
        console.log('[AgentResearcher] 🔄 Phase change detected:', {
            researchPhase,
            shouldReset: researchPhase === 'initializing' || researchPhase === 'extracting',
            beforeReset: hasAppendedResultsRef.current
        });
        
        // Reset when research STARTS, not when it ends
        if (researchPhase === 'initializing' || researchPhase === 'extracting') {
            hasAppendedResultsRef.current = false;
            console.log('[AgentResearcher] ✅ Reset append flag - NEW RESEARCH STARTING');
        }
    }, [researchPhase]);
